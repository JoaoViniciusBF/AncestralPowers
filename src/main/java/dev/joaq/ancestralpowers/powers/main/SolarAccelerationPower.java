...
public class SolarAccelerationPower extends PowerBase {
    ...
    private void tick(ServerPlayerEntity player) {
        ...
        if (!"Solar".equals(traits.getMainPower())) return;
        ...
        if (!state.locked && currentTime != TARGET_TIME) {
            ...
            player.sendMessage(Text.literal("☀ O sol se manteve em seu zenith por 1 minuto."), false);
            ...
            if (state.lockTimer > 0) {
                ...
                player.sendMessage(Text.literal("☀ O tempo solar termina, o ciclo dia/noite foi restaurado."), false);
            }
        }
    }
    ...
    protected void executeLogic(ServerPlayerEntity player, boolean activate, float stamina) {
        ...
        if (currentTime == TARGET_TIME) {
            player.sendMessage(Text.literal("☀ Já é meio-dia! O sol será mantido por 1 minuto."), false);
            return;
        }

        player.sendMessage(Text.literal("☀ Acelerando o tempo até o meio-dia... (3x)"), false);
    }
